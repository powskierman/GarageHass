...
class GarageRestManager: ObservableObject {
    ...
    private func processState(_ entity: HAEntity) {
        ...
        case "cover.leftdoor_left_garage_door":
            leftDoorClosed = entity.state == "closed"
        case "cover.rightdoor_right_garage_door":
            rightDoorClosed = entity.state == "closed"
    ...
    func toggleSwitch(entityId: String) {
        ...
        print("[GarageRestManager] Toggling \(domain) for \(entityId)")
        lastCallStatus = .pending
        HassRestClient(baseURL: baseURL, authToken: authToken).callService(domain: domain, service: service, entityId: entityId) { [weak self] result in
            DispatchQueue.main.async {
                switch result {
                case .success():
                    print("[GarageRestManager] Successfully toggled \(domain) \(entityId)")
                    self?.handleCompletionSuccess()
                    self?.fetchInitialState()
                case .failure(let error):
                    print("[GarageRestManager] Error toggling \(domain) \(entityId): \(error)")
                    self?.handleCompletionFailure(error)
                }
            }
        }
